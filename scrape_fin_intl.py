...
야후는 두 시장 모두 분기(또는 반기) 매출·영업이익·순이익을 몇 년치 준다.
공식 원본은 아니지만 **회사가 낸 숫자를 옮겨 적은 것**이고, 우리가 하려는 건
"매출이 늘고 있나 줄고 있나"를 그림으로 보는 것이라 이 정도면 맞다.
출처는 화면에 그대로 적는다 — SEC 인 척하지 않는다.
...
def series(sym):
    ...
    # 한 회사 안에 3M 과 6M 이 섞여 오는 수가 있다(중간에 보고 주기를 바꾼 회사).
    # 섞어서 그리면 반기 막대가 분기의 두 배로 솟아 추세를 왜곡한다. 많은 쪽만 남긴다.
    spans = {}
...
def main():
    ...
    payload = {
        ...
        "note": ("공식 원본이 아니라 야후가 옮겨 적은 값이다. 홍콩은 반기 보고가 "
                 "많아 반기 막대로 낸다. 통화는 회사가 보고한 통화 그대로다."),
        "count": len(have),
        "stocks": stocks,
    }
