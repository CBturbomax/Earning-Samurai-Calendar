...
**같은 목록에서 월매출(月次)도 같이 건진다.** 일본 회사 상당수가 분기 실적과 별개로
...
# 월매출은 캘린더 줄이 아니라 따로 쌓는다 — 성격도 화면 자리도 다르다.
MONTH_OUT = HERE / "data" / "monthly_jp.json"

# 담는 형식이 바뀌면 올린다. 그래야 받아둔 헌 기록을 버리고 다시 받는다.
#
# **월매출을 붙이면서 이 번호를 올리지 않았다.** 올리면 캐시가 통째로 비워져
# 다시 받는 십몇 분 동안 캘린더의 일본 발표 완료분이 얇아진다 — 월매출을 채우자고
# 멀쩡한 것을 깎을 이유가 없다. 대신 아래 main() 이 **두 캐시를 따로 보고**
# 월매출이 없는 날만 다시 훑는다. 결산단신 쪽은 같은 값으로 덮어써지므로 안 준다.
TDNET_VER = 2
MONTH_VER = 1
...
# 売上の状況に関するお知らせ（速報）」 — 월매출 공시인데 제목에 '月次' 가 아예
# 없다. '月次' 만 찾던 시절에는 통째로 새어나갔다.
MON_STRONG = re.compile(r"月次|月商|月例|月別")
MON_MONTH = re.compile(r"(\d{1,2})\s*月(?!期)")
# 매출 말고 다른 KPI 만 내는 회사가 있다 — 「2026年10月期 ８月度 主要KPI
# （会員数及び店舗数）についてのお知らせ」(212A). 전수 감사(40차)에서 이것만
# 안전하게 건질 수 있었다.
MON_SALES = re.compile(r"売上|販売|受注|業績|実績|概況|速報|稼働|来店|客数|入場|"
                       r"発電電力量|取扱|会員数|店舗数|KPI|ＫＰＩ")
# 결산단신·정정·예상수정은 월매출이 아니다. 특히 「業績予想の修正」 은 '業績' 이
# 들어 있어 위 낱말 규칙에 걸리므로 반드시 먼저 쳐내야 한다.
# 「説明資料」도 뺀다. 分기 실적설명 슬라이드가 「2026年9月期第2四半期
# （2025年10月～2026年3月）業績説明資料」 처럼 와서 '業績'+'N월' 규칙에 걸렸다 —
# 어드밴스크리에이트(8798)가 그렇게 월매출로 둔갑했고, 발표일이 들쭉날쭉해
# '다음 예상일'까지 엉뚱하게 나왔다. 월매출은 説明資料 라고 쓰지 않는다.
MON_SKIP = re.compile(r"決算短信|決算説明|説明資料|説明会|訂正|有価証券報告書|"
                      r"質疑応答|業績予想|配当予想|補足説明")
...
# 결산단신 쪽은 zip 을 따로 받으므로 안 쓰지만, 월매출은 **이 PDF 가 알맹이 전부**라
...
def is_monthly(title: str) -> bool:
    """월매출 공시인가. 실측 양성 36건·음성 15건으로 맞춰 둔 규칙이다."""
    t = title.translate(ZEN)
...
def fetch_day(day: str, probe: bool = False):
    """그날 목록을 한 번 훑어 **결산단신**과 **월매출**를 함께 건진다.

    돌려주는 것은 `(결산단신 줄, 월매출 줄)`. 첫 쪽부터 없으면 `None` —
    ...
    while page <= MAX_PAGE:
        ...
        if not rows:
            break
        for r in rows:
            # 월매출은 결산단신과 **다른 줄**이라 같은 반복문에서 따로 담는다.
...
def load_monthly():
    ...
    # 규칙으로 한 번 더 거른다. 실제로 어드밴스크리에이트(8798)의 분기
    # 실적설명자료 두 건이 이 문으로 빠져나갔다.
    by_day, dropped = {}, 0
    for r in old.get("rows", []):
        if not is_monthly(r.get("title", "")):
            dropped += 1
            continue
        by_day.setdefault(r["date"], []).append(r)
    if dropped:
...
def main(back_days: int, probe: bool = False):
    ...
    if probe:
        for back in range(3):
            ...
            if got:
                print(f"  -> 결산단신 {len(got[0])}건 · 월매출 {len(got[1])}건")
                for r in got[1]:
