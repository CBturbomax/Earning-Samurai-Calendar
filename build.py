...
def seg_fit(rec, fin_rec):
    ...
    # **넘치는 이유가 둘인데, 하나만 잘못이다.**
    #   (1) 같은 표가 두 벌 실렸다 — 부문을 제품축·지역축으로 두 번 쪼갠 것을
    #       함께 담았거나 같은 줄이 두 번 있다. 그러면 합이 총매출의 **정확히**
    #       두 배(네 배)가 되고 분기마다 그 값이 흔들리지 않는다. 실측: 테이크투
    #       4.00(퍼짐 0.00) · 워터스 2.00(0.00) · 사우스웨스트 1.99(0.00) ·
    #       위스콘신에너지 2.03(0.03).
    #   (2) 부문을 **상계 전 총액**으로 낸다 — 부문끼리 판 것이 섞여 있어 합이
    #       총매출보다 크다. 정상 공시다. 실측: 엑슨 1.40 · 비자 1.38 ·
    #       유나이티드헬스 1.36 · 인텔 1.29 · 셰브런 2.25 · 길리어드 2.65.
    #       배수가 어중간하고, 회사마다 값이 제각각이다.
    # 예전에는 "1.25 넘고 고르면 버린다"로 둘을 한꺼번에 버렸다. 그 바람에
    # 엑슨·비자·인텔·셰브런·유나이티드헬스를 포함한 **134종목이 통째로 사라졌다.**
    # 지금은 (1)만 버린다 — 정수배에 바짝 붙고 퍼짐이 거의 없을 때.
    k = round(med)
    if k >= 2 and abs(med - k) <= 0.06 and hi - lo <= 0.15:
        return None                            # 같은 표가 두 벌이다. 싣지 않는다.

    # "총매출의 몇 %" 는 두 수치가 서로 아귀가 맞을 때만 적는다. 흔들리는데
    # 적으면 틀린 근거로 적는 셈이다. 100%를 넘는 값도 그대로 적는다 —
    # 부문끼리 주고받은 것이 섞였다는 뜻이고, 숨기면 막대 높이를 총매출로
    # 오해한다.
...
def pack_seg(rec, fin_rec):
    ...
    # 100% 아래면 '부문이 매출 전부를 설명하지 않는다'(본사 몫·기타), 위면
    # '부문끼리 주고받은 것이 섞였다'는 뜻이다. 둘 다 적어야 막대 높이를
    # 총매출로 오해하지 않는다.
    if med is not None and (med < 0.95 or med > 1.05):
...
def build():
    ...
    if monthly:
        ...
        nm = sum(len(v["m"]) for v in monthly_nums.values())
        nrev = sum(1 for v in monthly_nums.values() for r in v["m"] if r[1] is not None)
        print(f"  🇯🇵 월매출 {len(monthly):,}건 / {len(mcodes):,}개사 / "
              f"{len(mper)}개월치")
        print(f"     수치 {len(monthly_nums):,}개사 / {nm:,}개월 "
              f"(금액 {nrev:,} · 나머지는 전년비만)")
...
.chip {
  ...
  box-shadow:inset 5px 0 0 0 var(--mk,transparent);
}
/* 시장 구분이 칩 색의 전부다 — 일본 빨강, 미국 파랑, 홍콩 노랑(markets.py).
   왼쪽 띠 + 코드 글자색 + 옅은 배경 틴트. 순위별 색칠은 하지 않는다. */
@supports (background: color-mix(in srgb, red 8%, black)) {
  .chip { background:color-mix(in srgb, var(--mk, #0f1620) 7%, #0f1620); }
  .chip:hover { background:color-mix(in srgb, var(--mk, #16202b) 13%, #16202b); }
}
...
.chip .cd {
  font-size:15px; font-weight:700; color:var(--mk,#8fb8dc);
...
function chip(r) {
  ...
  const we = wd >= 5
    ? '<span class="we" title="' + dateOf(r) + ' (' + DOW_KO[wd] + ') 발표">' +
      DOW_KO[wd] + '</span>' : '';
  return '<button class="chip m-' + r[9] +
         (got ? ' done' : '') +
...
function renderCal() {
  ...
  cal.innerHTML = shown.map(d => {
    ...
    if (!list.length) {
      ...
      const miss = missing(d), soon = notYet(d);
      // 상위 종목만 빨갛게 칠하던 것은 걷어냈다 — 회원님 지시다. 칩의 색은
      // 순위가 아니라 **시장 구분**(왼쪽 띠·코드색·옅은 배경)에만 쓴다.
      // 시총 순서 자체는 정렬이 이미 말해 준다.
      body = (hol.text ? '<div class="dsec gap">' + esc(hol.text) + '</div>' : '') +
             shownList.map(r => chip(r)).join('') +
...
const MN = D.monthly || [];
const MNUM = D.monthlyNum || {};

/* 다음 발표일 어림. **확정이 아니라 예상이다** — 회사가 월매출 예정일을 공식으로
...
/* 달별 막대. 금액이 있으면 금액을 그리고 전년비를 막대 위에 적는다.
   금액 없이 전년비만 내는 회사가 많아서(소매업이 특히), 그때는 100 을 기준선으로
   놓고 **위아래로** 그린다 — 0 부터 그리면 95% 와 105% 가 거의 같은 높이가 되어
   좋아졌는지 나빠졌는지가 안 보인다. */
/* **모든 카드가 같은 축을 쓴다 — 2024년 1월부터 가장 최근 달까지.**
   카드마다 제 자료의 첫 달부터 그리면 옆 카드와 달이 어긋나 나란히 놓고
   비교할 수가 없고, 빠진 달을 건너뛰고 붙여 그리면 2월 다음이 5월인데도
   옆칸에 서서 구멍이 안 보인다(전년동월비 선도 이어진 것처럼 거짓말을 한다).
   축을 달로 고정하면 있는 달은 있는 자리에, 없는 달은 빈자리로 보인다. */
...
function segChart(sg, cur) {
  ...
  return '<div class="finhead sub">' + esc(sg.ax || '사업부') + '별 매출' +
    ...
    (sg.cov ? ' · 총매출의 ' + sg.cov + '%' +
       (sg.cov > 105 ? ' — 부문끼리 주고받은 것 포함' : '') : '') + ')</span></div>' +
